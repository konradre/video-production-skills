...
| hero vs its flat | `normalise_shots.py verify`: 10-bit, the flat's raster and frame count, the flat starting at pts 0, mean \|dY\| > 1/255 (the look reached the file), a (0, 0) phase correlation of the luma (no crop, scale, weave or warp), and the hero's middle frame matching the flat's frame m rather than m ± 1 | a flat that started at 0.041 s: Resolve rendered one extra leading frame and every hero sat one frame late (2026-09-26); a look that never applied |
| the normalise | hero saturation and clipping per shot against the target: median chroma of the client shots ≈ the approved grade's, clipping ~0 % | mapping the percentiles to 0/1 clipped 7–30 % under the look's own gain; the Dehancer hero landed ~27 % under the cube's saturation estimate (heroes 0.0249 against 0.0302 until `--t-scale 0.91`) |
...
| vs the previous version | `--prev <file>`: each footage event's middle frame against the same timeline frame of the previous version and its two neighbours — **regraded** (mean \|dY\| > 1/255, phase (0, 0), best structural match at m), **moved** (no neighbour lines up at (0, 0): a crop, scale or warp), **retimed** (a neighbour lines up: a one-frame slip), **same**; `--prev-expect finish` fails anything but regraded, `--prev-expect changed:<ids>` fails a named event that reads same | a finish that doubled the first frame of 14 of 22 events — a half-frame in-point into the hero quantised to a rounding tie in the builder's `fps` filter — while every per-hero row passed, and the approved version's own four doubled first frames (in-points at x.2 / x.4 of a frame) that read `retimed` until the in-points took the nearest frame (2026-09-26); a changed square crop that a builder's id-keyed cache never re-rendered |
| platform contract | INFO: the colour range tag (`tv`), the keyframe spacing (a closed 2 s GOP = every 48 frames at 24 fps), the edit lists (0) — look-library GUIDE § 7 | a validator that rejects an open or irregular GOP; an A/V offset on a platform that ignores the edit list the muxer wrote for the AAC priming |
