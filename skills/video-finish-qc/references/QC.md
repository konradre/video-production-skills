...
| true peak | ≤ the platform ceiling (−1 dBTP); the EDL's TP is the encode target, exceeded by ≤ ~1 dB of AAC overshoot | a hot transient past a static gain with no limiter; a limiter placed only AFTER the resample (measured −1.0 dBTP against −2.0 before / −2.1 both) |
...
| black / frozen frames | no black run ≥ 0.1 s except a trailing fade; no frozen run ≥ 0.5 s inside the footage span (the card holds by design) | a hole from a missing layer or a truncated stage; a stalled overlay |
