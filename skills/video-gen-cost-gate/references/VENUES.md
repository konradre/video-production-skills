...
| **Higgsfield API · `higgsfiled/genjutsu/{object-swap,motion-transfer}/v1.0`** — video → video on an EXISTING clip, added 2026-09-24 (⚠ `higgsfiled` is the vendor's own spelling in `GET /models` — copy it verbatim) | two dedicated endpoints: **`object-swap`** (a subject or object in the source becomes a reference) · **`motion-transfer`** (the source's motion drives a reference). The catalog's `input_schema` is `null` and the docs carry no page, so the body was READ from the estimator's own validation errors (2026-09-24): **`video_url`** required · **`image_urls` 1–8** required (`[]` → *"should be non-empty"*, 9 → *"too long"*) · `prompt` optional string · `resolution` `480p\|720p`. 🔴 **Unknown fields are ACCEPTED SILENTLY** (`zzz_unknown` → 200), so a `duration`, a ratio or a typo looks sent and does nothing — `hf_api_submit.py` sends only these four | **per second of INPUT video, rounded UP: $0.318/s @480p · $0.681/s @720p, LIST** (*"before any applicable customer discount"*; none recorded, so the gate quotes list). 🔴 `/estimate` returns that sentence and **NO NUMBER, even for a real hosted video** (measured) — the gate probes the source's length and computes. ≈ 1.29× a same-length Seedance 2.5 `video-edit` at 480p ((in + gen) × $0.1234 = $0.2468/s list) | ❓ **the input-duration cap is UNREAD** — the estimator never fetches the video, so the first receipt (or a free refusal) settles it; output fps, audio pass-through and whether two identities land in ONE pass are UNMEASURED | UNMEASURED on this model. The same API refused a public figure on Seedance (`nsfw`, $0, 2026-09-18) and accepted an ordinary face; moderation runs DURING processing, so a clean submit is not the verdict. `failed` / `nsfw` are not charged (API-wide) | `hf_api_submit.py --model genjutsu --mode object-swap\|motion-transfer --source <clip NAME> --refs A[,B…]` — the source is the gate's start image (import the client's clip with its provenance), and its length is read with ffprobe from the local file `hf-api-urls.json` records; `hf_api.py quote --model genjutsu --input-seconds N` |
...
- 🟢 **Genjutsu — re-casting footage we ALREADY hold** (`object-swap` / `motion-transfer`, added 2026-09-24).
  Every other row generates from references; this one swaps a subject into an existing clip, or drives a reference
  with that clip's motion. Per INPUT second, $0.318 at 480p, list — its row in the Video table carries the schema.
...
|---|---|---|---|
| **kie · `google/gemini-omni-flash-1-1`** — **the FIRST Omni venue (since 2026-09-25); fal, the next row, is the fallback.** Omni's default model and the newer release (27 Aug 2026) — **Omni itself is the default ONLY for a UGC talking-head clip (since 2026-09-26); every other shot stays on Seedance 2.5**; `gemini-omni-video` beside it is the original Omni release: the same price and inputs, without 360p and first/last frame | `POST https://api.kie.ai/api/v1/jobs/createTask`, polled on `recordInfo` — the jobs API `gen_stills.py` already speaks; `duration` **4 / 6 / 8 / 10 s ONLY** — a line between two steps buys the next one and pads its tail (`video-prompt-dialects` PHONE-NATIVE.md § Dialogue); `aspect_ratio` 16:9 / 9:16; `resolution` 360p (1.1 only) / 720p / 1080p / 4k; the inputs share **7 units** — an image 1, a character id 1 (≤ 3), the one video 2, and a video input sets the duration itself; `first_frame_url` / `last_frame_url` (1.1 only) EXCLUDE every other input; `audio_ids` is a DESIGNED voice from `gemini-omni-audio` (a preset Gemini voice + a description + an example line) and `character_ids` comes from `gemini-omni-character` (a portrait + a body image + a voice) — neither takes an uploaded recording | **flat across 360p / 720p / 1080p: 4 s $0.315 · 6 s $0.42 · 8 s $0.525 · 10 s $0.63**; 4k $0.735 → $1.05; any video input $0.84 per generation ($1.26 at 4k) — read free from kie's own pricing 2026-09-25, no receipt yet; kie exposes no balance endpoint | the fal row's model at ~0.66× fal's 720p price and ~0.44× its 1080p price; one designed voice held across per-line clips by `audio_ids` (vendor-documented, unmeasured here — the model still invents words, so a locked VO still routes to the audio-driven default); 1080p costs the same as 720p here, and whether Google's own upscale beats our 1.5× lanczos is unmeasured; submitted by `scripts/gen_video_kie.py` (the refs gate on the kie ledger, the step price beside fal's, the task ledger written at creation, `kie_poll.py` detached), the fal fallback by `gen_video_fal.py --engine omni` |
| **fal · `google/gemini-omni-flash/v1.1/{image-to-video,reference-to-video,text-to-video,edit}`**; monid `gemini /v1/video/omni-flash-*` — the Omni FALLBACK since 2026-09-25, submitted by `gen_video_fal.py --engine omni` | 3–10 s integer; 16:9 / 9:16; **720p NATIVE** (1080p and 4k are Google's own upscale ⇒ generate at 720p and reconstruct up); r2v ≤ 10 image refs (fal's schema, re-read 2026-09-25) + ≤ 3 video refs of ≤ 3 s; **NO audio input on any surface** — a reference clip's audio is ignored | fal **$0.03 / 0.10 / 0.15 / 0.30 per s** at 360p / 720p / 1080p / 4k (monid +$0.01); MEASURED: a 7 s 720p 9:16 i2v = 23.39 fal units, ~44 s wall; refusals bill nothing; fal result URLs expire, monid's keep 7 days | the simple talking head and the hook sweep (360p drafts at $0.03/s for remixes of one clip); MP4 with native audio + SynthID; ~15 % of takes stutter → regenerate; it INVENTS words (a client line came back with "dollars" on two numbers, and "say exactly these words" made it worse) ⇒ a script-exact mouth needs an audio-driven model (fal `fal-ai/bytedance/omnihuman/v1.5`, image + audio) or a lipsync pass over the take (`fal-ai/sync-lipsync/v3`, Kling lipsync, HeyGen v3 — their prices render client-side and are UNREADABLE in advance: the `x-fal-billable-units` header measures them); a negation BURNED captions onto the subject's shirt — positive-spec only |
...
for one" on fal (≈ $0.62 and $3.15 on kie, where a 7 s line buys the 8 s step), and a Seedance 2.5 720p 30 s pass is ≈ $8.85 BEFORE retries — the cost line quotes the usable figure with its
...
with no locked VO a UGC talking head → Omni Flash 1.1, kie first (§ The talking-head default), and a commercial one →
Seedance 2.5; a UGC hook sweep → Omni 1.1 at 360p — the one Omni job fal does cheaper, $0.03/s against kie's flat step
price, so quote both;
product-in-hand or a 30 s story → Seedance 2.5 (the label is drawn, never read — keep it as an image reference and the
label shot short and front-on); a defined handheld move → Seedance 2.5 too (Kling 3.0 ≤ 8 s is unmeasured here, the row
above); the words that must land exactly → an audio-driven model or a lipsync pass, never a prompt. 🔴 **Omni is the
default for UGC talking-head clips ONLY (since 2026-09-26)** — b-roll, product, equipment, a scene and an
image-to-video from a still stay on Seedance 2.5 whatever Omni's step price. Every take
