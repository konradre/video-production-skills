...
| 1a′ | reference teardown — a format the team has not measured | `ad-spot-preprod` `references/REFERENCE-TEARDOWN.md` + `scripts/reference_teardown.py` | "tear down a reference", "like this one", a new format before its shot list | the instrument's self-test; 3–5 real references (one is a copy study); the measured columns never replaced by a model's estimate | the references, the client's script, the runtime | `TEARDOWN.md` (measured shots, the written read, kept/swapped), the beat grid → the shot list's partitions |
| 1c | footage curation — the client supplies the picture | `ad-spot-preprod` `references/FOOTAGE-CURATION.md` (instruments in `video-take-review` and `video-edit-edl`) | a pack of client clips, "which shots do we use", "pick the b-roll" | the frame-rate decision made against its printed motion cost; every instrument's self-test; nothing conformed before the pick | the pack, the script, the programme audio | the route table, the slots, the plan file with a runner-up per slot, the proxy preview and its check sheet, the gap and fact-check lists |
