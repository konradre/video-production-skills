...
| `footage_intake.py --src <dir\|files> --out <dir> [--aspect 9:16] [--delivery-width 1080] [--id-regex] [--overview 12] [--survey-every 4] [--no-allframes] [--no-hash]` · `--selftest` | a PACK of supplied clips, per clip: sha256 + bytes, the display raster, the centred delivery crop, cut candidates (the `qc_seed` rule, lifted from its source so it stays single-sourced), per-frame shift by phase correlation at 320 wide, mean RGB steps, and three sheet tiers — `overview.jpg` (12 samples: which clips are one camera route), `survey-NN.jpg` (every 4th frame: what is in frame over the whole clip), `allframes-NN.jpg` (every frame: a window's in and out) — all SURVEY images; streams, so a long clip never sits in memory | motion is image displacement, never camera-versus-subject. **The conform cadence:** a frame is a double step at ≥ 1.7× the median of its six neighbours, a held frame at ≤ 0.25×, counted only where the move is fast enough to tell; 10–40 % double steps = a same-speed conform from a faster source (29.97 → 24 measured 0.235 against 0.25 predicted). **Shake is read on the axis the move does NOT use:** the smoothed residual ALONG a pan sat at ~27 % of pan speed on every pan of a dropped-frame pack while the other axis read 0.02–0.3 px — it was measuring the cadence. Mean RGB steps move with composition: they locate a step for the eye. Self-tests: static → 0,0 · a planted 3,−2 shift · a +20 brightness and a +15 red step · the 1-1-1-2, even, every-5th-held and static cadences · one hard cut found, sustained motion not a cut |
| `window_metrics.py --root --candidates <json> --out [--intake <dir>] [--aspect 9:16] [--raster 1080x1920] [--stack]` · `--selftest` | candidate WINDOWS read at the delivery shape — one decode through the crop and the raster the deliverable will use: Laplacian sharpness, mean luma, % clipped high and low, Hasler–Süsstrunk colourfulness, speed in frame-widths/s, the step in px at the delivery width (mean, p90), the cadence on that window, shake on the unused axis; a 5-frame strip per candidate (survey), `--stack` one sheet per slot | a table, never a total score. Sharpness compares ONLY files of one camera route showing the same thing — texture (gravel, foliage) inflates it, a smooth subject filling the frame deflates it (a tight close-up read 265 beside 1,000–6,000 for wider frames of the same sharp footage). The step at the delivery width is the looks number on conformed footage: ≤ ~5 px does not read, ~30 px stutters. Self-tests: flat → 0 · a checkerboard > 1e5 · grey colourfulness 0 · pure red **85.53** (the first draft of that test expected > 100: the expectation was wrong, the instrument was right) |
...
| `mouth_open.py <take> --box x0,y0,x1,y1 [--ref-raster 768x1344] [--model <face_landmarker.task>] [--early 1.5] [--sheet] [--json]` · `--selftest` | ONE face's mouth-open ratio per frame — the inner-lip gap over the mouth width (MediaPipe FaceLandmarker points 13–14 over 78–308) on the box crop upscaled to 480 px: the max in the first 1.5 s and when (the silent-subject flap), the median and max after 2 s, the FOUND frame count, and `--sheet` the enlarged crop at 0–1.5 s and 3 s (a verdict image) | calibrated 2026-09-19 on one local H3 scene, 36 takes, the eye-read sheets agreeing take by take: start stills caught mid-word 0.31 (median of six), the mouth closed 0.19, plus a concrete soundscape 0.15; closed lips read ~0.03–0.10. FOUND under 90 % means a wrong box or a face too small: fix the box, never read the ratio. A dark-pixel mouth box, tried first, did not separate the arms. The box is given in `--ref-raster` pixels and scales to the take, so one box reads a 768p pass and its 1088p refine. `--selftest`: the ratio on synthetic lips (0 / 0.20 / 0.35) and the box scaling — detection has no synthetic known answer, which is why FOUND and the sheet ride beside every read |
| `invented_sound.py <take>... [--after 1.5] [--model small] [--band 300,3400] [--thr 0.005] [--json]` · `--selftest` | speech a joint audio-video model INVENTED on a take that should carry none, after `--after` s: faster-whisper words (VAD on, no-speech probability under 0.6) and the RMS of the 300–3400 Hz band; INVENTED when either fires | calibrated 2026-09-19 on one local H3 scene: the band read 0.051–0.053 on every take with invented speech and 0.0001–0.0007 on every clean one, room tone with a named hum included; it reproduced a by-ear count of 30 takes over five arms (2 / 0 / 1 / 1 / 0) take for take. A scripted line, a bed or an effect in the band is not invented — read the words beside the number. `--selftest`: room noise under the threshold, a syllable-rate speech-band burst over it |
...
| dead-flat blocks — the share of 8×8 blocks with zero variance, beside the noise floor and the median block sd (`video-finish-qc` `phone_texture_probe.py`, five native centre crops per clip, the mean and the frame RANGE) | the texture band of a frame — read as a MATCHED-CONTENT comparison against the project's own real footage, never as a threshold | a measurement, not a look decision: 27.8 % of every frame dead-flat at a 0.00 noise floor on one Seedance → Rhea → grade master, and grain injected at σ 0.5/1/2/4 read back 0.45/0.90/1.78/3.50 with the share at 0 % — linear. But two REAL phone clips as they arrived from a client read 0–10 % dead-flat at a 0.1–0.7 floor, and three raw 720p Omni Flash takes read 0 % at 1.8–2.0 — MORE fine texture than the phone clips (2026-09-16): "zero floor = generated" holds only on matched content and a matched encode; the band varies by scene class inside one clip (6 % and 34 % on two frames of the same clip); whether the texture moves, and in which direction, is `video-finish` § 5's call (the phone-native tier), dosed by this probe |
...
- A self-test that FAILS is worked by hand before the instrument is touched: the known answer may be the thing that
  is wrong.
- A pack of supplied clips is ROUTES, not files: one subject seen by one camera move, often recorded two or three times
  in different aspect modes. Rank shots, never files (`ad-spot-preprod/references/FOOTAGE-CURATION.md` § 4).
